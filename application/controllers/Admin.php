...
class Admin extends CI_Controller {
	...
	{	
		...
		if(isset($_POST['add_staff'])){
			...
			$exists_uid = $this->Admin_model->userid_exists($_POST['userid']);	
			
			if($exists || $exists_uid){
				if ($exists){
					$this->session->set_flashdata('msg1', 'Mobile Number already exist.');					
				}elseif ($exists_uid){
					$this->session->set_flashdata('msg1', 'User Id already exist.');					
				}
				$role_id= $_SESSION['role'];
				$data['uname'] = $_SESSION['username'];
				$data['menus']= $this->Admin_model->get_menus($role_id);
				...
				if($_FILES['adr_proof']['tmp_name']!="")
					move_uploaded_file($file_tmp1, $dest1);
				$id = $this->Admin_model->add_StaffData($data['refresh'],$mobile);
	...
	public function update_staff_data()
	{
		...
		if(isset($_POST['update_staff'])){
			...
			if($_FILES['id_proof']['tmp_name']!="")
			{
				...
				$url = "assets/uploads/".$_POST['c_id_name'];				
				if(file_exists($url))
				{
			...
			if($_FILES['adr_proof']['tmp_name']!="")
			{
				...
				$img1="assets/uploads/".$file_name1;
				$img_url1=base_url().$img1;
				$url1 = "assets/uploads/".$_POST['c_add_name'];
				if(file_exists($url1))
				{
					unlink($url1);
				}
