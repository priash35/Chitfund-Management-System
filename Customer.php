...
class Customer extends CI_Controller {
	...
	{			
		...
		{			
			...
			$count = count($exists);
			  if(empty($count))
			  {
				  $cust_id = $this->Customer_model->add_cust_basic($data);	
	...
	public function add_bankdata()
	{
		...
		if(isset($_POST['add_bank']))
		{
			...
			$exists = $this->Customer_model->bank_exists($bank,$account);
			$count = count($exists);
			  if(empty($count))
			  {
	...
	public function update_bankdata()
	{
		$uname = $_SESSION['username'];
			...
			$data= array(
				...
				'branch_name' => $_POST['br_name'],
				'account_number' => $_POST['a_num'],
				'ifsc_code' => $_POST['if_code'],
				'updated_date' => $updated_date,
				'updated_by' => $uname		
			);
		...
		if(isset($_POST['add_docs']) && !empty($_FILES['documents']['name']))
		{
            ...
            for($i = 0; $i < $filesCount; $i++){
                ...
                if($this->upload->do_upload('userFile')){
					...
					$uploadData[$i]['created_by'] = $uname;
					$uploadData[$i]['updated_by'] = $uname;
	...
	public function DeleteDocument()
	{
		...
		redirect("Customer/add_documents",$data);
		//$this->load->view('admin/customer-master',$data);
	}
